...
| `template_id`       | string          | No       | -             | Template ID to use for this crawl                                                                                        |
| `variables`         | object          | No       | -             | Template variables (only used when `template_id` is provided)                                                            |
| `engine`            | enum            | No       | `auto`        | Per-page scraping engine: `auto`, `cheerio`, `playwright`, `puppeteer`                                                   |
...
| `store_in_cache`    | boolean         | No       | `true`        | Whether to write Page Cache for per-page scrapes                                                                          |
| `retry`             | boolean         | No       | `false`       | Whether to retry on failure                                                                                               |
| `proxy`             | string (URI)    | No       | -             | Optional proxy URL                                                                                                        |
| `formats`           | array of enum   | No       | `["markdown"]`| Output formats for per-page scrapes                                                                                       |
| `timeout`           | number          | No       | `60000`       | Per-request timeout (ms)                                                                                                  |
| `wait_for`          | number          | No       | -             | Delay before extraction (ms); browser engines only                                                                        |
| `wait_until`        | enum            | No       | -             | Navigation wait condition for browser engines: `load`, `domcontentloaded`, `networkidle`, `commit`                        |
| `wait_for_selector` | string, object, or array | No | -          | Wait for one or multiple selectors (browser engines only). Takes priority over `wait_for`.                               |
| `include_tags`      | array of string | No       | -             | Only include elements that match CSS selectors                                                                            |
| `exclude_tags`      | array of string | No       | -             | Exclude elements that match CSS selectors                                                                                 |
| `only_main_content` | boolean         | No       | `true`        | Only extract main content, removing headers, footers, navigation, etc.                                                    |
| `json_options`      | object          | No       | -             | Options for structured JSON extraction (schema, user_prompt, schema_name, schema_description)                             |
| `extract_source`    | enum            | No       | `markdown`    | Source to extract JSON from: `markdown` (default) or `html`                                                               |
| `ocr_options`       | boolean         | No       | `false`       | Enable OCR enhancement for markdown images only. Affects markdown generation, not `html`/`rawHtml`.                       |
...
- Crawl currently does **not** read Page Cache, but it may still write.
- If you pass `scrape_options`, set cache controls inside it.
