---
title: MCP Server
description: Integrate AnyCrawl web scraping into Cursor, Claude, and VS Code using the Model Context Protocol (MCP)
icon: Zap
---
...
- `engine` (optional): Scraping engine (`auto`, `playwright`, `cheerio`, `puppeteer`). Defaults to `auto`
...
- `ocr_options` (optional): Enable OCR enhancement for markdown images only; does not modify `html`/`rawHtml`
...
- Automatically picks the optimal engine per URL
