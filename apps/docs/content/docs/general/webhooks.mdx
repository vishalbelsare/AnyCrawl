---
title: Webhooks
description: Receive real-time notifications for all AnyCrawl events including scraping, crawling, mapping, search, and scheduled tasks.
icon: Webhook
---
...
- **Event Subscriptions**: Subscribe to scraping, crawling, mapping, search, scheduled task, monitor, and system events
...
| Event | Description | Triggered When |
|-------|-------------|----------------|
| `monitor.check.completed` | Check completed | A monitor run finishes (scheduled or on-demand) |
| `monitor.changed` | Content changed | Webpage monitor detects a meaningful text change |
| `monitor.price.changed` | Price changed | Price monitor detects a structured field change |
| `monitor.error` | Monitor error | Monitor check fails |

Monitor payloads include inline `diff_text`, `diff_json`, and optional AI `judgment` so you can act without fetching the changes API. See [Monitors](./monitors) for setup and payload examples.
...
| Event | Description | Triggered When |
|-------|-------------|----------------|
| `map.created` | Map job created | New map job is queued |
| `map.started` | Map job started | Job begins execution |
| `map.completed` | Map job completed | Job finishes successfully |
| `map.failed` | Map job failed | Job encounters an error |
...
<Callout type="info">
Webhooks are automatically disabled after 10 consecutive failures to prevent excessive retries. You can reactivate them manually after fixing the issue.
</Callout>
...
- [Scheduled Tasks](./scheduled-tasks) - Automate recurring jobs
- [Scrape API](./scrape) - Scraping endpoints
- [Crawl API](./crawl) - Crawling endpoints
