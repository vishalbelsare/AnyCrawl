...
**后续 CI 补充（PR #99）：** 下文记录修复验收时的本地测试与构建结果。提交后，Docker amd64 与 Vercel 均因新增通知集成测试遗漏 `drizzle-orm` 直接依赖而编译失败；本地解析到仓库外的同名依赖掩盖了该问题。干净安装与完整镜像构建尚未通过，不能将下文的本地构建结果视为远端 CI 通过。根因、影响和修复计划见 [PR #99 构建错误审查](./pr-99-ci-errors-2026-09-06.md)。
